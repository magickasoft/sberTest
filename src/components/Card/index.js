...
class Card extends React.Component{
	...
	render() {
		...
		let wrapperClass =
			status === 'visited' ? cn(styles.cardWrapper, styles.cardVisited) : styles.cardWrapper
		...
		if (cardUrl.indexOf('://') !== -1) {
			return (
				<a href={cardUrl} target="_blank" className={wrapperClass} onMouseEnter={this.onHover} onMouseLeave={this.outHover}>
					{component.header}
					{!component.header &&
						header &&
						header.url &&
						type.label !== 'test' &&
						type.position !== 'horizontalFull' && <Header overlay={this.state.isHover} {...header} position={type.position} parent={parent}/>}
					<div className={styles.card}>
						...
						<div className={styles.cardContent}>
							...
							{!component.content &&
								type.label !== 'test' &&
								type.position !== 'horizontalFull' && <Content type={type} content={content} />}
							{!component.content &&
								(type.label === 'test' || type.position === 'horizontalFull') && (
		...
		return (
			<NavLink to={cardUrl} className={wrapperClass} onMouseEnter={this.onHover} onMouseLeave={this.outHover}>
				...
				{!component.header &&
					header &&
					header.url &&
					type.label !== 'test' && type.label !== 'service' && 
					type.position !== 'horizontalFull' && <Header overlay={this.state.isHover} {...header} position={type.position} parent={parent}/>}
				{!component.header &&
					type.label === 'service' &&
					<HeaderImage {...header} card='simple' alt={content.title}/>
				}
				<div className={styles.card}>
					{component.aside}
					{!component.aside && aside && <Aside {...aside} title={content.title}/>}

...
export default class Slick extends React.Component {
	...
	onResize = () => {
		...
		for (let item of currentState.options.responsive) {
		...
		return (
			<div className={css.root}>
				...
				</Slider>
				{
					(
						options.infinite || 
						(!options.infinite && this.state.slideId !== 0) 
					) &&
...

				{
					(
						options.infinite || 
						(!options.infinite && this.state.slideId < (this.props.children.length - options.slidesToShow))
					) &&

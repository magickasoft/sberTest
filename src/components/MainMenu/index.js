...
class MainMenu extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			submenuCoords: this.props.menu.map(el => {
				return { left: 0, top: 0 }
			}),
			submenuStyles: this.props.menu.map(el => {
				return { left: 0, top: 0 }
			}),
		...
		currentState.isSubmenuOpen.forEach((el, i) => {
			if (el) {
				...
				currentState.submenuStyles[i] = {
					left:
						currentState.submenuCoords[i].left +
						(parentElCoords.left - currentState.submenuCoords[i].left),
	...
	clickOutside = ev => {
		if (!ev.target.classList.contains('subs')) {
			...
			currentState.submenuCoords = this.state.submenuCoords.map(el => {
				return { left: 0, top: 0 }
			});
			this.setState({
				isSubmenuOpen: currentState.isSubmenuOpen,
						...
						{menu.map((el, i) => {
							if (el.url) {
								const linkCss = cn(css.menuContentItem, {
									[css.menuLinkActive]: el.url === this.props.currentRoute,
								});
									...
									<NavLink to={el.url} key={i} className={linkCss} activeClassName={css.menuLinkActive}>
										{el.icon && (
											<Icon
												...
												type={el.icon}
												size={16}
												{...(el.url === this.props.currentRoute ? { grad: true } : {})}
											/>
										)}
